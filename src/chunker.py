...
SKIP_DIRS = {".git", "__pycache__", ".mypy_cache", "node_modules", ".venv"}
...
def splitter_func(file_path: str, max_chunk_size: int) -> list[ChunkData]:
    ...
    if ext in lang_map:
        splitter = RecursiveCharacterTextSplitter.from_language(
            chunk_size=max_chunk_size, chunk_overlap=max_chunk_size // 5, 
            language=lang_map[ext], add_start_index=True
        )
    else:
        splitter = RecursiveCharacterTextSplitter(
            chunk_size=max_chunk_size, chunk_overlap=max_chunk_size // 5, add_start_index=True
        )
